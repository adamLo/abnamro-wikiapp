...
+ (instancetype)wmf_placesActivityWithURL:(NSURL *)activityURL {
    ...
    // This will add a location to activity
    ...
    for (NSURLQueryItem *item in components.queryItems) {
        if ([item.name isEqualToString:@"WMFArticleURL"]) {
            ...
            if (coordinatesArray.count == 2) {
                NSNumber *latitude = [coordinatesArray objectAtIndex:0];
                NSNumber *longitude = [coordinatesArray objectAtIndex:1];
                if (latitude && longitude) {
                    MKPlacemark *placeMark = [[MKPlacemark alloc] initWithCoordinate:CLLocationCoordinate2DMake(latitude.doubleValue, longitude.doubleValue)];
